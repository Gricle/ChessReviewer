    ...
    <div className="glass-panel rounded-2xl p-6 border border-indigo-400/20 flex flex-col shadow-xl space-y-4">
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          ...
          >
            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : t('platform.load')}
          </button>
        </div>
        ...
        <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
          ...
          {loaded && games.length === 0 && (
            <p className="text-[11px] text-slate-500 font-mono px-1 py-1">{t('platform.noGames')}</p>
          )}
        </div>
      </div>
    </div>
...
export function ImportSection({ onPgn }: Props) {
  ...
  async function loadPlatform(
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t('platform.loadFailed'));
    } finally {
      setLoading(null);
    }
  }
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
          <span className="text-xs font-mono text-slate-400 mr-1">{t('hero.quickDemos')}</span>
          {Object.entries(SAMPLE_PGNS).map(([key]) => {
            const sampleKey = key as keyof typeof SAMPLE_PGNS;
            const sample = SAMPLE_PGNS[sampleKey];
            return (
              <button
                key={key}
                onClick={() => onPgn(sample.pgn, 'paste')}
                title={t(`samples.${sampleKey}.sub`)}
              ...
              >
                <Sparkles className={`w-3 h-3 ${SAMPLE_ICON_COLOR[sampleKey]}`} /> {t(`samples.${sampleKey}.label`)}
              </button>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel rounded-2xl p-6 border border-indigo-400/20 flex flex-col justify-between shadow-xl space-y-4">
          ...
          >
            ...
            <span>{t('paste.submit')}</span>
          </button>
        </div>

        {/* Column 2: From Chess.com */}
        <PlatformImportCard
          title={t('platform.chesscom.title')}
          iconAccent="text-emerald-400"
          description={t('platform.chesscom.description')}
          placeholder={t('platform.chesscom.placeholder')}
          value={comUser}
        ...
        <PlatformImportCard
          title={t('platform.lichess.title')}
          iconAccent="text-amber-400"
          description={t('platform.lichess.description')}
          placeholder={t('platform.lichess.placeholder')}
          value={liUser}
